...
# def chat():
#     # Expecting JSON input with a "messages" array:
#     # {
#     #   "messages": [
...
#         logger.warning("Invalid request: empty or non-list messages")
#         return jsonify({"error": "A non-empty 'messages' array is required"}), 400

#     try:
#         # Call the new 1.0+ openai interface:
#         # client.chat.completions.create(...)
#         #
#         # model="gpt-4o" is a placeholder for your custom model name.
#         # store=True will store the conversation in your OpenAI account history (optional).
#         logger.info("Initiating OpenAI chat completion with model=gpt-4o")
#         response = client.chat.completions.create(
#             model="gpt-4o",
#             messages=messages,
#             store=True,
#             stream=True  # Enable streaming of partial chunks
#         )

#         # Stream chunks of the response back to the client
#         def generate():
#             logger.info("Starting response stream")
#             chunk_count = 0
#             try:
#                 for chunk in response:
#                     # Each chunk looks like: {"choices": [{"delta": {"content": "..."} }], ...}
#                     delta = chunk["choices"][0].get("delta", {})
#                     if "content" in delta:
#                         chunk_count += 1
#                         yield delta["content"]
#                 logger.info(f"Stream completed successfully. Sent {chunk_count} chunks")
#             except Exception as e:
